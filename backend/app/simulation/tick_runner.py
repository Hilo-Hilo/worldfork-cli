...
def run_next_tick(db: Session, *, multiverse: models.Multiverse, idempotency_key: str | None = None, force: bool = False) -> models.TickSnapshot:
    ...
    existing = db.scalar(select(models.TickSnapshot).where(models.TickSnapshot.idempotency_key == key))
    if existing and not force:
        return existing

    config = db.scalar(
        select(models.BigBangConfig).where(models.BigBangConfig.big_bang_id == multiverse.big_bang_id).order_by(models.BigBangConfig.version.desc()).limit(1)
    )
    simulation_config = config.simulation_config if config else {}
    ...
    if next_index > max_ticks:
        ...
        raise ValueError("timeline has reached max_ticks")
    ...
    executed_events = execute_due_events(db, due_events)
    ...
    tick = models.TickSnapshot(
        ...
        status="provisional",
        provisional_bundle=provisional,
        final_bundle={},
        summary=f"Tick {next_index} simulated for {multiverse.ui_label}.",
        idempotency_key=key,
    )
    db.add(tick)
    db.flush()
    ...
    for call in review_payload["tool_calls"]:
        ...
        tool_results.append({"tool_name": tool.tool_name, "status": tool.status, "result": tool.result, "error": tool.error})
